...
// Interruptor en línea / fuera de línea (requiere cuenta validada)
router.patch("/me/estado", requireValidado, validate({ body: estadoSchema }), async (req, res, next) => {
  try {
...
router.get("/me/solicitudes", requireValidado, async (req, res, next) => {
  try {
